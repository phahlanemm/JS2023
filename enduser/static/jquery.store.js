(function( $ ) {
    ...
    $.Shop.prototype = {
        init: function() {
             // Properties
        this.cartPrefix = "moe-"; // Prefix string to be prepended to the cart's name in the session storage
        ...
        handleAddToCartForm: function() {
            ...
            // Event delegation for dynamic and static content
            this.$element.on("submit", "form.add-to-cart", function(event) {
                event.preventDefault();
                var $form = $(this);
                var $product = $form.closest(".product");
                var $product1 = $form.closest(".product-top");
                var $product2 = $form.closest(".product");
                var price = self._convertString($product.find(".product-description").data("price"));
                var name = $product.find(".product-description").data("name");
                var prodcode = $product.find(".product-description").data("prodid");
                ...
                var sTotal = total + subTotal;
                       // Ensure that prodcode, name, and price are retrieved correctly


                self.storage.setItem(self.total, sTotal);
                console.log('Item added. Prod Code:' + prodcode + ', Tot:'+subTotal+ ',Product:'+name+',price:'+price+',QTY:'+qty);
                self._addToCart({
                    prodid: prodcode,
                    product: name,
                    price: price,
                ...
                // Extract the navigation URL from the button's data attribute
        var navigationURL = $form.find('input[data-navigation-url]').data('navigation-url');

        if (navigationURL) {
            // Navigate to the specified URL
            window.location.href = navigationURL;
        }
                // Add code to update the cart display or any other actions you need
                // Example: self.displayCart();
            });
        }
        ,
